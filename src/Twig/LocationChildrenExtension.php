...
final class LocationChildrenExtension extends AbstractExtension
{
    ...
    public function getChildren(Location $location, int $limit = 50): array
    {
        ...
        $query->sortClauses = [
            new \eZ\Publish\API\Repository\Values\Content\Query\SortClause\Location\Priority(),
            new \eZ\Publish\API\Repository\Values\Content\Query\SortClause\ContentName(),
        ];
        $query->limit = $limit;
